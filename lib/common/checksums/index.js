...
class Checksums {
  ...
  compile(compileType, stage) {
    this.treeFile = path.join(this.bsq.vars.app.path, '.build', 'tree.json');
      ...
      this.data.get(compileType, stage).then((previousChecksumData) => {
        ...
        _.forEach(functions, (functionObject) => {
          ...
          if (previousChecksumData
            ...
            const functionChecksumData = _.find(previousChecksumData.checksums[0].functions,
              val => val.name === functionName);
